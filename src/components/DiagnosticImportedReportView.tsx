...
export function DiagnosticImportedReportView({
  ...
  return (
    <div className="space-y-10">
      <header className="border-b border-[#ebe5dc] pb-6">
        <p className="text-xs font-bold uppercase tracking-wide text-[#9a8b7e]">
          AI-analyzed diagnostic
        </p>
        <h2 className="onboarding-serif mt-1 text-2xl font-semibold text-[#2c2825] sm:text-3xl">
          {report.reportTitle}
        </h2>
        <p className="mt-2 text-sm text-[#7a6e66]">
          <span className="font-medium text-[#3d3835]">{report.sourceFileName}</span>
          <span className="text-[#9a8b7e]">
            {' '}
            ·{' '}
      ...
      <section className="grid gap-8 lg:grid-cols-[minmax(0,220px)_1fr] lg:items-start">
        <div className="flex flex-col items-center rounded-2xl border border-[#ebe5dc] bg-white/90 p-6 shadow-sm">
          <div
          ...
          >
            <div className="flex h-[5.5rem] w-[5.5rem] flex-col items-center justify-center rounded-full bg-[#faf7f3]">
              {report.totalScore != null ? (
                <>
                  <span className="onboarding-serif text-3xl font-bold tabular-nums text-[#2c2825]">
                    {report.totalScore}
                  </span>
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-[#7a6e66]">
                    Total
                  </span>
                </>
              ) : (
                <span className="px-2 text-center text-xs font-medium text-[#7a6e66]">
          ...
          {report.totalPercentile ? (
            <p className="mt-4 text-center text-sm font-semibold text-[#5f7f6a]">
              {report.totalPercentile}
            </p>
          ) : null}
        </div>
        ...
        <div>
          <h3 className="text-sm font-semibold uppercase tracking-wide text-[#9a8b7e]">
            Section scores
          </h3>
          <ul className="mt-4 space-y-4">
            {report.sections.length === 0 ? (
              <li className="text-sm text-[#9a8b7e]">
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        {s.scaledScore != null ? (
                          <span className="onboarding-serif text-xl font-semibold tabular-nums text-[#2c2825]">
                            {s.scaledScore}
                          </span>
                        ) : (
                          <span className="text-sm text-[#9a8b7e]">—</span>
                        )}
                        {s.percentile ? (
                    ...
                    {pct !== null ? (
                      <div className="mt-2">
                        <div className="mb-1 flex justify-between text-[10px] text-[#9a8b7e]">
                          <span>118</span>
                          <span>132 scale</span>
                        </div>
              ...
              <li key={row.name}>
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                  ...
                  {row.percentCorrect != null ? (
                    <span className="tabular-nums text-[#5f7f6a]">
          ...
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {report.passageHighlights.map((p, i) => (
              <li
                ...
                className="rounded-lg border border-[#ebe5dc] bg-[#faf9f7] px-3 py-2 text-sm"
              >
                <span className="text-xs font-semibold text-[#5f7f6a]">
                  {p.section}
                </span>
                <div className="mt-0.5 flex justify-between gap-2 text-[#3d3835]">
                  <span>{p.label}</span>
      ...
      <section className="rounded-2xl border border-[#d4e4f9] bg-[#f4f8fd] p-5 sm:p-6">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-[#1e3a5f]">
          Coach summary
        </h3>
        <div className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-[#2c2825]">
          {report.coachSummary}
        </div>
      </section>

      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-[#cfe5d6] bg-[#f4faf6] p-5">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-[#166534]">
            Strengths
          </h3>
          <ul className="mt-3 list-inside list-disc space-y-2 text-sm text-[#14532d]">
            {report.strengths.length ?
              report.strengths.map((s) => <li key={s}>{s}</li>)
            : <li className="list-none text-[#7a6e66]">—</li>}
          </ul>
        </section>
        <section className="rounded-2xl border border-[#fecaca] bg-[#fff5f5] p-5">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-[#b91c1c]">
            Growth areas
          </h3>
          <ul className="mt-3 list-inside list-disc space-y-2 text-sm text-[#7f1d1d]">
      ...
      {report.studyPriorities.length > 0 ? (
        <section className="rounded-2xl border border-[#e8dfd4] bg-white/90 p-5">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-[#9a8b7e]">
            Suggested study priorities
          </h3>
          <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm leading-relaxed text-[#3d3835]">
      ...
      <details className="rounded-xl border border-[#e8dfd4] bg-[#faf9f7] p-4 text-xs">
        <summary className="cursor-pointer font-semibold text-[#7a6e66]">
          Extracted text preview (debug)
        </summary>
        <p className="mt-3 whitespace-pre-wrap break-words leading-relaxed text-[#9a8b7e]">
          {report.textExcerpt}
        </p>
      </details>
    </div>
