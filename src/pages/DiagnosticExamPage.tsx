...
export function DiagnosticExamPage() {
  ...
  if (phase === 'results') {
    ...
    return (
      <AppShell active="diagnostic">
        <div className="mx-auto flex max-w-2xl flex-col px-5 py-10 md:px-8 md:py-14">
          ...
          <section className="mb-8 rounded-2xl border border-white/40 bg-white/25 p-8 text-center shadow-[0_8px_32px_rgba(31,38,135,0.15)] backdrop-blur-xl md:p-10">
            <p className="text-sm font-semibold uppercase tracking-wide text-[#9a8b7e]">
              Raw score
            </p>
            <p className="onboarding-serif mt-2 text-5xl font-semibold tabular-nums text-[#2c2825] md:text-6xl">
              {correct}
              <span className="text-2xl font-medium text-[#9a8b7e]">
                {' '}
                / {total}
              </span>
            </p>
            <p className="mt-2 text-sm text-[#6b5f56]">{pct}% correct</p>
          </section>
  ...
  return (
    <AppShell active="diagnostic">
      <div className="mx-auto flex max-w-3xl flex-col px-5 pb-28 pt-6 md:px-8 md:pb-16 md:pt-8">
        ...
        <div className="mb-6 h-2 w-full overflow-hidden rounded-full bg-[#e8dfd4]">
          <div
        ...
        <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-[#9a8b7e]">
          Section {sectionIdx + 1} of {DIAGNOSTIC_SECTIONS.length} ·{' '}
          {section.shortName}
        </p>
        <h1 className="onboarding-serif mb-2 text-xl font-semibold leading-snug text-[#2c2825] md:text-2xl">
          {section.fullName}
        </h1>
        <p className="mb-8 text-sm text-[#8a7b70]">
          Question {qInSection + 1} of {section.questions.length} in this
          section
        </p>

        <section className="rounded-2xl border border-white/40 bg-white/25 p-6 shadow-[0_8px_32px_rgba(31,38,135,0.15)] backdrop-blur-xl md:p-8">
          {question.passage ? (
            <div className="mb-6 rounded-2xl border border-white/40 bg-white/30 p-5 text-sm leading-relaxed text-[#4a423c] backdrop-blur-lg">
              {question.passage}
            </div>
          ) : null}
          <p className="text-base font-medium leading-relaxed text-[#2c2825] md:text-[1.05rem]">
            {question.stem}
          </p>
              ...
              return (
                <li key={i}>
                  <button
                    type="button"
                    onClick={() => setChoice(i)}
                    className={`flex w-full rounded-2xl border-2 px-4 py-4 text-left text-sm font-semibold leading-snug transition-all md:px-5 md:py-4 md:text-[0.95rem] ${
                      on
                        ? 'border-[#5f7f6a] bg-white/70 text-[#2d3f32] shadow-[0_4px_20px_-8px_rgba(95,127,106,0.3)]'
                        : 'border-white/30 bg-white/35 text-[#4a423c] shadow-sm hover:bg-white/50'
                    }`}
                  >
                    <span
                      className={`mr-3 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 text-xs tabular-nums ${
                        on
                          ...
                          : 'border-[#d4c8bc] bg-white text-[#6b5f56]'
                      }`}
                    >
        ...
        <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-white/30 bg-white/45 px-5 py-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] backdrop-blur-2xl md:static md:mt-10 md:border-0 md:bg-transparent md:p-0 md:pb-0 md:backdrop-blur-none">
          <div className="mx-auto flex max-w-3xl justify-between gap-4">
            <button
              type="button"
              onClick={goBack}
              className="py-2.5 text-sm font-semibold text-[#6b5f56] hover:text-[#2c2825]"
            >
              Back
            </button>
            <button
              ...
              disabled={!canAdvance}
              onClick={goNext}
              className="rounded-full bg-[#2c2825] px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#1f1c1a] disabled:cursor-not-allowed disabled:opacity-45"
            >
